		...
		while IsDead do
			...
			if obezwladniony then
				...
				if signal < GetGameTimer() then
					if IsDisabledControlPressed(0, Keys['G']) and not LocalPlayer.state.IsHandcuffed and not LocalPlayer.state.Tied then
						SendDistressSignal()
...
function SendDistressSignal()
	if IsDead then
		ESX.TriggerServerCallback('base:hasphone', function(callback)
			if callback then
			else
				ESX.ShowNotification("Nie posiadasz telefonu")
			end
		end)
	end
end
...
function RemoveItemsAfterRPDeath()
	CreateThread(function()
		...
		ESX.TriggerServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function()			
			...
			DoScreenFadeIn(800)
			TriggerServerEvent('wait_logs:triggerLog', "Przeteleportował się na szpital: "..hospital, 'teleportszpital')
	...
	CreateThread(function ()
		...
		if weapon == `WEAPON_UNARMED` or ((weapon == `WEAPON_RUN_OVER_BY_CAR` or damagedbycar) and sourceofdeath ~= playerPed) or weapon == `WEAPON_NIGHTSTICK` then
			...
			Citizen.SetTimeout(knockoutDuration, function() 
				RespawnPed(PlayerPedId(), GetEntityCoords(PlayerPedId()))
				Citizen.Wait(500)
				SetEntityHealth(PlayerPedId(), 170)
				TriggerServerEvent("esx_ambulancejob:setDeathStatus", 0)
				obezwladniony = false -- zmienione
			end)
		end
...
-- 		options = {
-- 			{
-- 				event = "wait_organizations:Fixkit",
-- 				icon = "fas fa-wrench",
-- 				label = "Napraw pojazd [ambulance]",
-- 				hasJob = false,
...
function OpenCloakroomMenu()
	...
	if PlayerData.job.grade >= 14 then
		elements[#elements + 1] = {label = 'Ubrania LSFD', value = 'lsfd_wear'}
	end

	if PlayerData.job.name == 'ambulance' then
		elements[#elements + 1] = {label = 'Ubrania Służbowe', value = 'alluniforms'}
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		if data.current.value == 'ambulance_wear' then
			...
			if PlayerData.job.name == 'offambulance' then
				TriggerServerEvent('wait:setJob', 'ambulance', true)
		...
		if data.current.value == 'citizen_wear' then
			...
			if PlayerData.job.name == 'ambulance' then
				...
				TriggerServerEvent('wait:setJob', 'ambulance', false)
			end
		...
		if data.current.value == 'lsfd_wear' then
			local elements2 = {
				{label = "Pomocnik straży", value = 'lsfd8_wear'},
				{label = "Strażak", value = 'lsfd7_wear'},
				{label = "Doświadczony Strażak", value = 'lsfd6_wear'},
				{label = "Elitarny Strażak", value = 'lsfd5_wear'},
				{label = "Kapitan", value = 'lsfd4_wear'},
				{label = "Brygadier", value = 'lsfd3_wear'},
				{label = "Zastępca Szefa", value = 'lsfd2_wear'},
				{label = "Szef LSFD ", value = 'lsfd1_wear'},
			}

			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'lsfd_wear', {
				title    = "Szatnia - LSFD",
				align    = 'right',
				elements = elements2
			}, function(data2, menu2)
	...
	if
		...
		data.current.value == 'neurochirurg_wear' or
		data.current.value == 'lsfd1_wear' or
		data.current.value == 'lsfd2_wear' or
		data.current.value == 'lsfd3_wear' or
		data.current.value == 'lsfd4_wear' or
		data.current.value == 'lsfd5_wear' or
		data.current.value == 'lsfd6_wear' or
		data.current.value == 'lsfd7_wear' or
		data.current.value == 'lsfd8_wear'
	then
...
if Config.Target then
	function OpenVehicleSpawnerMenu(partNum)
		local vehicles = Config.TargetAmbulanceStations.Vehicles
		...
		for i, group in ipairs(Config.VehicleGroups) do
			...
			if (PlayerData.job.name == 'ambulance' and PlayerData.job.grade >= 12) or (PlayerData.hiddenjob.name == 'sheriff' and PlayerData.hiddenjob.grade >= 11) then
				if #elements2 > 0 then
					elements[#elements + 1] = {label = group, value = elements2, group = i}		
				end
			else
				if i == 5 then
					found = false
		  ...
		  elements = elements
			...
			if type(data.current.value) == 'table' and #data.current.value > 0 then
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner_' .. data.current.group, {
					...
					local ParkingsToCheck = #vehicles[partNum].SpawnPoints
					local RandomVehicleSpawn = 1
					:: vehSpawnE ::
					local vehicle = GetClosestVehicle(vehicles[partNum].SpawnPoints[RandomVehicleSpawn].x,  vehicles[partNum].SpawnPoints[RandomVehicleSpawn].y,  vehicles[partNum].SpawnPoints[RandomVehicleSpawn].z, 2.0, 0, 71)
					if DoesEntityExist(vehicle) then
						RandomVehicleSpawn = RandomVehicleSpawn + 1
						if RandomVehicleSpawn > ParkingsToCheck then
							ESX.ShowNotification('~r~Wszystkie miejsca parkingowe są zajęte')
							return
						end
						goto vehSpawnE
					end
					local CoordsToSpawn = {
						["x"] = vehicles[partNum].SpawnPoints[RandomVehicleSpawn].x,
						["y"] = vehicles[partNum].SpawnPoints[RandomVehicleSpawn].y,
						["z"] = vehicles[partNum].SpawnPoints[RandomVehicleSpawn].z,
						["h"] = vehicles[partNum].SpawnPoints[RandomVehicleSpawn].h
					}
					if not DoesEntityExist(vehicle) then
						...
						if Config.MaxInService == -1 then
							ESX.Game.SpawnVehicle(data2.current.model, {
								...
								y = CoordsToSpawn.y,
								z = CoordsToSpawn.z
							}, CoordsToSpawn.h, plate, true, function(vehicle)
								...
								if setPlate then
									...
									local localVehPlate = tostring(string.lower(GetVehicleNumberPlateText(vehicle)))
	...
	for i, group in ipairs(Config.VehicleGroups) do
		...
		for _, vehicle in ipairs(Config.AuthorizedVehicles) do
			...
			if let then
				...
				if let then
					elements[#elements + 1] = { label = vehicle.label, model = vehicle.model, livery = vehicle.livery, extrason = vehicle.extrason, extrasoff = vehicle.extrasoff, offroad = vehicle.offroad, wheelsxd = vehicle.wheelsxd, color = vehicle.color, plate = vehicle.plate, tint = vehicle.tint, bulletproof = vehicle.bulletproof, wheel = vehicle.wheel, tuning = vehicle.tuning }
				end
			end
		end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner_' .. data.current.group, {
				...
				if not DoesEntityExist(vehicle) then
					...
					if Config.MaxInService == -1 then
						...
						}, vehicles[partNum].heading, plate, true, function(vehicle)
							SetVehicleMaxMods(vehicle, livery, offroad, wheelsxd, color, data2.current.extrason, data2.current.extrasoff, bulletproof, tint, wheel, tuning)
...
function OpenHeliSpawnerMenu(zoneNumber)
	...
	'default', GetCurrentResourceName(), 'heli_spawner',
	{
		...
		menu.close()
		ESX.Game.SpawnVehicle(data.current.model, Config.Ambulance.Helicopters[zoneNumber].spawnPoint, Config.Ambulance.Helicopters[zoneNumber].heading, plate, true, function(vehicle)
			local playerPed = PlayerPedId()
			local plate = "EMS " .. math.random(100,999)
			SetVehicleNumberPlateText(vehicle, plate)
			local localVehPlate = tostring(string.lower(GetVehicleNumberPlateText(vehicle)))
			TriggerEvent('ls:dodajklucze2', localVehPlate)
			TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
...
function OpenBoatSpawnerMenu(zoneNumber)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boat_spawner',
	{
		...
		ESX.Game.SpawnVehicle(data.current.model, Config.Ambulance.Boats[zoneNumber].spawnPoint, Config.Ambulance.Boats[zoneNumber].heading, plate, true, function(vehicle)
			local playerPed = PlayerPedId()
			local plate = "EMS " .. math.random(100,999)
			SetVehicleNumberPlateText(vehicle, plate)
			local localVehPlate = tostring(string.lower(GetVehicleNumberPlateText(vehicle)))
...
AddEventHandler('hypex_ambulancejob:hypexrevive', function(notBlock)
	...
	if IsDead and not notBlock then
		TriggerServerEvent('wait_wypadanie:bron')
								...
								ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'save_dressing',
								{
									...
									if data2.current.value == 'yes' then
										ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'outfit_name', {
											...
											TriggerEvent('skinchanger:getSkin', function(skin)
												TriggerServerEvent('wait_freeproperty:saveOutfit', data3.value, skin)
...
function refreshBlip()
	if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'ambulance' then
		for _, ez in ipairs(Config.OnlySamsBlip) do
			...
			AddTextComponentString("Łodzie - EMS")
...
function OpenInventoryMenu()
	...
	-- 		TriggerEvent('wait:getInventoryItem', 'society_ambulance')
	-- 	elseif data.current.value == 'put' then
	-- 		TriggerEvent('wait:putInventoryItem', 'society_ambulance')
	-- 	end
	-- end, function(data, menu)
	-- 	menu.close()
	-- 	CurrentAction		= 'items_menu'
	...
	if ox_inventory:openInventory('stash', "ems_storage") == false then
	  TriggerServerEvent('esx_ambulancejob:loadStash', "ems_storage", vector3(310.902435, -602.540283, 43.433910))
	  ox_inventory:openInventory('stash', "ems_storage")
	end
end
...
function OpenInventoryMenu2()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'items', {
		...
		menu.close()
		if data.current.value == 'get' then
			TriggerEvent('wait:getInventoryItem', 'society_sams2')
		elseif data.current.value == 'put' then
			TriggerEvent('wait:putInventoryItem', 'society_sams2')
		end
	end, function(data, menu)
		menu.close()
		CurrentAction		= 'items2_menu'
...
function OpenAmbulanceDutyList()
	ESX.TriggerServerCallback('esx_ambulancejob:getDutyList', function(list)
		...
		for i, data in pairs(list) do
			elements[#elements + 1] = { label = data.label, value = i }
		end
...
function ProcessNewPosition()
    ...
    local maxRadius = 3.5
    if (hitBool and #(vec3(pCoords.x, pCoords.y, pCoords.z + 0.5) - hitCoords) < 3.5 + 0.5) then
        maxRadius = #(vec3(pCoords.x, pCoords.y, pCoords.z + 0.5) - hitCoords)
    end
...
Citizen.CreateThread(function()
	if Config.Target then
		...
		-- exports.qtarget:AddTargetModel({-969349845}, {
		-- 	options = {
		-- 		{
		-- 			event = "esx_ambulancejob:target:bossmenu",
		...
		-- 				return GetEntityCoords(entity) == vec3(-785.48, -1244.35, 7.17)
		...
		exports.qtarget:AddBoxZone("PillboxBossMenu", vector3(-674.02, 312.11, 92.74), 1.00, 2.5, {
		...
		exports.qtarget:AddBoxZone("PillboxCloackroom", vector3(-664.97, 323.93, 93.18), 1.15, 6.5, {
			name = "PillboxCloackroom",
			heading = 355.0,
			debugPoly = false,
			minZ = 92.68 - 0.87,
			maxZ = 93.08 + 1.24,
			...
			options = {
				{
					event = "esx_ambulancejob:target:ambulanceactions",
					icon = "fas fa-shirt",
					label = "Otwórz szafkę",
					num = 1
				},
				{
					event = "esx_ambulancejob:target:skinmenu",
					icon = "fas fa-person",
					label = "Otwórz kreator",
					job = "ambulance",
					num = 2
				},
			},
		...
		-- exports.qtarget:AddBoxZone("PillboxPharm", vector3(-820.32, -1243.47, 7.33), 0.85, 2.2, {
		-- 	name = "PillboxPharm",
		-- 	heading = 140.0,
		-- 	debugPoly = false,
		-- 	minZ = 7.33 - 0.87,
		...
		-- 	options = {
		-- 		-- {
		-- 		-- 	event = "esx_ambulancejob:target:pharmacy",
		-- 		-- 	icon = "fas fa-briefcase",
		-- 		-- 	label = "Otwórz Aptekę",
		-- 		-- 	num = 1
		-- 		-- },
		-- 		{
		-- 			event = "esx_ambulancejob:target:inventorymenu",
		-- 			icon = "fas fa-briefcase",
		-- 			label = "Otwórz Szafkę",
		-- 			num = 2
		-- 		}
		...
		-- exports.qtarget:AddTargetModel({854291622}, {
		-- 	options = {
		-- -- 		{
		-- -- 			event = "esx_ambulancejob:target:pharmacy",
		-- -- 			icon = "fas fa-briefcase",
		-- -- 			label = "Otwórz Aptekę",
		-- -- 			canInteract = function(entity)
		-- -- 				return GetEntityCoords(entity) == vec3(310.902435, -602.540283, 43.433910)
		-- -- 			end
		-- -- 		},
		-- 		{
		-- 			event = "esx_ambulancejob:target:inventorymenu",
		...
		-- 			label = "Otwórz Szafkę",
		-- 			canInteract = function(entity)
		-- 				return GetEntityCoords(entity) == vec3(-820.24, -1243.700, 7.33)
		-- 			end
		-- 		}
		-- 	},
		-- 	distance = 1.5
		...
		-- VEHICLE SPAWNER/DELETERS ALL
		for k, v in pairs(Config.TargetAmbulanceStations.Vehicles) do
			local PedConfig = v.SpawnerPed
			RequestModel(joaat(PedConfig["hash"]))
			while not HasModelLoaded(joaat(PedConfig["hash"])) do
				Wait(0)
			end

			local SpawnedPed = CreatePed(2, PedConfig["hash"], PedConfig["x"], PedConfig["y"], PedConfig["z"] - 1.0, PedConfig["h"], false, true)
			...
			exports.qtarget:AddTargetEntity(SpawnedPed, {
				options = {
					{
						event = "esx_ambulancejob:target:openPoliceGarage",
						icon = "fas fa-car",
						label = "Otwórz garaż",
						job = "ambulance",
						num = 1,
					...
					{
						event = "esx_ambulancejob:target:hidePoliceVehicle",
...
RegisterNetEvent("esx_ambulancejob:target:bossmenu", function(data)
	if PlayerData.job.grade_name == 'boss' then
		TriggerEvent('esx_society:openBossMenu', 'ambulance', function(data, menu)
			menu.close()
		end, { showmoney = true, withdraw = true, deposit = true, wash = false, employees = true, badges = true})
...
RegisterNetEvent("esx_ambulancejob:target:openPoliceGarage", function(data)
	OpenVehicleSpawnerMenu(data.id)
end)

RegisterNetEvent("esx_ambulancejob:target:hidePoliceVehicle", function(data)
	if not VehicleToDelete or not DoesEntityExist(VehicleToDelete) then
		VehicleToDelete = GetVehiclePedIsIn(PlayerPedId(), true)
	end
	local vehicles = Config.TargetAmbulanceStations.Vehicles
	local ParkingsToCheck = #vehicles[data.id].SpawnPoints
	local DeleteVehicleAttempts = 1

	:: vehDeleteE ::
	local vehicle = GetClosestVehicle(vehicles[data.id].SpawnPoints[DeleteVehicleAttempts].x,  vehicles[data.id].SpawnPoints[DeleteVehicleAttempts].y,  vehicles[data.id].SpawnPoints[DeleteVehicleAttempts].z, 2.0, 0, 71)
	if VehicleToDelete == vehicle and DoesEntityExist(VehicleToDelete) then
		...
		DeleteVehicleAttempts = DeleteVehicleAttempts + 1
		if DeleteVehicleAttempts > ParkingsToCheck then
			ESX.ShowNotification('~r~Nie znaleziono żadnego pojazdu możliwego do schowania!')
			return
		end
		goto vehDeleteE
	end
...
RegisterNetEvent("esx_ambulancejob:target:ambulanceactions", function()
	OpenAmbulanceActionsMenu()
end)

RegisterNetEvent("esx_ambulancejob:target:skinmenu", function()
	OpenShopMenu()
end)

RegisterNetEvent("esx_ambulancejob:target:pharmacy", function()
	OpenPharmacyMenu()
end)

RegisterNetEvent("esx_ambulancejob:target:inventorymenu", function()
	OpenInventoryMenu()
end)