        ...
        if next(interactions, nil) == nil then Wait(2000) else
            for index, v in pairs(interactions) do
                ...
                if meters <= Shared.DrawDistance then
                    sleep = false
                    if Shared.UseNopixelLikeSprite == true then
                        DrawSprite('4.0_inspired', np, screenX, screenY + 0.1, 0.021, 0.030, 0, circlecolor[1], circlecolor[2], circlecolor[3], 500)
                    else
