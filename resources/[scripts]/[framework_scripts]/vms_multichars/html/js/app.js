        ...
        $.each(characters, function(index, char){
            if (char) {
                if (char.disabled == 1) {
                    $(`#char-${char.id}`).html("");
                    setTimeout(function(){
                        $(`#char-${char.id}`).html(`<p id="slot-name"><i class="fa-solid fa-lock"></i></p>`);
                        $(`#char-${char.id}`).data('cData', char)
                        $(`#char-${char.id}`).data('cid', char.id)
                        $(`#char-${char.id}`).data('disabled', true)
                    }, 100)
                } else {
                    $(`#char-${char.id}`).html("");
                    setTimeout(function(){
                        $(`#char-${char.id}`).html(`<p id="slot-name">${char.id}</p>`);
                        $(`#char-${char.id}`).data('cData', char)
                        $(`#char-${char.id}`).data('cid', char.id)
            ...
            $('[data-charid=1]').html(`
                <div class="character-info">
                    <p class="character-info-name"> 
                        <span class="font_charname">${data.firstname} ${data.lastname}</span>
                    </p>
                    <p class="character-info-work">
                        <i class="fa-solid fa-suitcase"></i>
                        <span class="font_description">${data.job} - ${data.job_grade}</span>
                    </p>
                    <p class="character-info-money">
                        <i class="fa-solid fa-wallet"></i>
                        <span class="font_description">${money.format(data.money)}</span>
                    </p>
                    <p class="character-info-bank">
                        <i class="fa-solid fa-building-columns"></i>
                        <span class="font_description">${money.format(data.bank)}</span>
                    </p>
                    <p class="character-info-dateofbirth">
                        <i class="fa-solid fa-calendar-days"></i>
                        <span class="font_description">${data.dateofbirth}</span>
                    </p>
                    <p class="character-info-gender">
                        <i class="fa-solid fa-venus-mars"></i>
                        <span class="font_description">${data.sex}</span>
                    </p>
                </div>
        ...
        if (selectedChar !== null) {
            if (charData !== "") {
                $.post('https://vms_multichars/selectCharacter', JSON.stringify({
                    cData: $(selectedChar).data('cData').id
                }));
            } else {
