...
window.addEventListener('message', function(event) {
    ...
    if (event.data.type == "display") {
        $("body").fadeIn();
        document.getElementById("top-menu").innerHTML = '<a href="#all" onclick="menuVehicle(event)" value="all" class="selected">WSZYSTKO</a>';
    ...
    if (event.data.type == "updateVehicleInfos") {
        ...
        $('#contentVehicle').append(`
            ...
            <div class="row" id="actionButton">
                <a class="buyButton" onclick="openModalMenu()">Kup</a>
...
function menuVehicle(event) {
    ...
    dataVehicles = [] 
    $.post('https://waitrp_vehicleshop/menuSelected', JSON.stringify({menuId: categoryVehicleSelected.toLowerCase()}));
}

function testDrive(){
    $.post('https://waitrp_vehicleshop/TestDrive', JSON.stringify({vehicleModel: vehicleSelected.modelcar}));
...
function buyVehicle(option) {
    ...
    switch(option) {
        case 'cancel':
            break;
        case 'confirm':
            $.post('https://waitrp_vehicleshop/Buy', JSON.stringify(vehicleSelected));
            break;
    }
}
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27:
            $.post('https://waitrp_vehicleshop/Close');
            $('body').css("display", "none");
        ...
        case 17:
            break;
        case 68:
            $.post('https://waitrp_vehicleshop/rotate', JSON.stringify({key: "left"}))
            break;
        case 65:
            $.post('https://waitrp_vehicleshop/rotate', JSON.stringify({key: "right"}))
            break;
        case 39: 
...
$(document).on('keydown', function(ev) {
    ...
    if (_key) {
        $.post('https://waitrp_vehicleshop/rotate', JSON.stringify({key: _key}))
    }
});
    ...
    switch(event.which) {
        ...
        case 1:
            ...
            if(!itemDisabled) {
                ...
                $.post("https://waitrp_vehicleshop/SpawnVehicle", JSON.stringify({ modelcar: dataCar[9].outerText, price: dataCar[7].outerText }));
